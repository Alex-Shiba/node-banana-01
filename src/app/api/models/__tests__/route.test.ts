...
describe("/api/models route", () => {
  ...
  describe("basic functionality", () => {
    it("GET: should return models from fal.ai when no Replicate key", async () => {
      ...
      // 2 fal models + 11 gemini models (3 image + 8 video, always included)
      ...
      expect(data.providers.gemini.count).toBe(11);
    });
    ...
    it("GET: should return models from both providers when both keys present", async () => {
      ...
      expect(response.status).toBe(200);
      expect(data.success).toBe(true);
      // 1 replicate + 1 fal + 11 gemini models (always included)
      expect(data.models).toHaveLength(13);
  ...
  describe("error handling", () => {
    it("GET: should handle partial provider failures gracefully", async () => {
      ...
      expect(response.status).toBe(200);
      expect(data.success).toBe(true);
      // 1 fal + 11 gemini models (always included)
  ...
  describe("fal.ai category mapping", () => {
    it("GET: should map fal.ai categories to ModelCapability", async () => {
      ...
      expect(response.status).toBe(200);
      // 4 fal models + 11 gemini models (always included)
    ...
    it("GET: should filter out non-relevant fal.ai categories", async () => {
      ...
      expect(response.status).toBe(200);
      // 1 fal text-to-image + 1 fal text-to-speech (mapped to text-to-audio) + 11 gemini models (always included)
      expect(data.models).toHaveLength(13);
  ...
  describe("sorting", () => {
    it("GET: should sort models by provider, then by name", async () => {
      ...
      // Gemini models: 3 image + 8 video, sorted by name
      const geminiNames = [
        "Gemini Omni Flash",
      ...
      geminiNames.forEach((name, i) => {
        expect(data.models[2 + i].provider).toBe("gemini");
        expect(data.models[2 + i].name).toBe(name);
      });
      expect(data.models[13].provider).toBe("replicate");
      expect(data.models[13].name).toBe("alpha");
      expect(data.models[14].provider).toBe("replicate");
      expect(data.models[14].name).toBe("zebra");
    });
  });
});
