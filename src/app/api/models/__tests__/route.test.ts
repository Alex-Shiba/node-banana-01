...
describe("/api/models route", () => {
  ...
  describe("basic functionality", () => {
    it("GET: should return models from fal.ai when no Replicate key", async () => {
      ...
      // 2 fal models + 9 gemini models (3 image + 6 video, always included)
      ...
      expect(data.providers.gemini.count).toBe(9);
    });
    ...
    it("GET: should return models from both providers when both keys present", async () => {
      ...
      expect(response.status).toBe(200);
      expect(data.success).toBe(true);
      // 1 replicate + 1 fal + 9 gemini models (always included)
      expect(data.models).toHaveLength(11);
  ...
  describe("error handling", () => {
    it("GET: should handle partial provider failures gracefully", async () => {
      ...
      expect(response.status).toBe(200);
      expect(data.success).toBe(true);
      // 1 fal + 9 gemini models (always included)
  ...
  describe("fal.ai category mapping", () => {
    it("GET: should map fal.ai categories to ModelCapability", async () => {
      ...
      expect(response.status).toBe(200);
      // 4 fal models + 9 gemini models (always included)
    ...
    it("GET: should filter out non-relevant fal.ai categories", async () => {
      ...
      expect(response.status).toBe(200);
      // 1 fal text-to-image + 1 fal text-to-speech (mapped to text-to-audio) + 9 gemini models (always included)
      expect(data.models).toHaveLength(11);
  ...
  describe("sorting", () => {
    it("GET: should sort models by provider, then by name", async () => {
      ...
      // Gemini models: 3 image + 6 video, sorted by name
      expect(data.models[2].provider).toBe("gemini");
      expect(data.models[2].name).toBe("Gemini Omni Flash");
      expect(data.models[3].provider).toBe("gemini");
      expect(data.models[3].name).toBe("Gemini Omni Flash I2V");
      expect(data.models[4].provider).toBe("gemini");
      expect(data.models[4].name).toBe("Nano Banana");
      expect(data.models[5].provider).toBe("gemini");
      expect(data.models[5].name).toBe("Nano Banana 2");
      expect(data.models[6].provider).toBe("gemini");
      expect(data.models[6].name).toBe("Nano Banana Pro");
      expect(data.models[7].provider).toBe("gemini");
      expect(data.models[7].name).toBe("Veo 3.1");
      expect(data.models[8].provider).toBe("gemini");
      expect(data.models[8].name).toBe("Veo 3.1 Fast");
      expect(data.models[9].provider).toBe("gemini");
      expect(data.models[9].name).toBe("Veo 3.1 Fast I2V");
      expect(data.models[10].provider).toBe("gemini");
      expect(data.models[10].name).toBe("Veo 3.1 I2V");
      expect(data.models[11].provider).toBe("replicate");
      expect(data.models[11].name).toBe("alpha");
      expect(data.models[12].provider).toBe("replicate");
      expect(data.models[12].name).toBe("zebra");
    });
  });
});
